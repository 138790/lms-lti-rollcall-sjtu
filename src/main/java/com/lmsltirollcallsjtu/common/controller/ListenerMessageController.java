...
//            System.out.println("========spring-boot-queue========== receive msg : " + msgStr + "，消息ID：" + orderId);
//
...
//            String redisOrderId = RedisUtil.getString(orderId);
//            if (!StringUtils.isEmpty(redisOrderId)) {
//                return;
//            }
//
//            //封装入参Map参数
//            Map orderParamsMap = JSONObject.parseObject(msgStr, Map.class);
//            //调用学生签到，如果调用失败则重试，超过重试次数，则转发给死信队列进行处理
////            scanSignServcie.scanSignServcie();
//
//            //用于解决幂等性问题（即重复消费问题）,redis写入之后一般立马查询一下确认是否存入成功，因为redis没有记录成功的话会导致重试时查询不到而通过，依然会导致重复消费
//            RedisUtil.setString(orderId, orderId, 2L, TimeUnit.DAYS);
//            String messageIdOfRedis = RedisUtil.getString(orderId);
...
//            String orderId = message.getMessageProperties().getMessageId();
//            String msgStr = new String(message.getBody(), "UTF-8");
//            System.out.println("========dead-letter-queue========== receive msg : " + msgStr + "，消息ID：" + orderId);
//
//            //封装入参Map参数
//            QuerySendOrderDetailParam querySendOrderDetailParam = new QuerySendOrderDetailParam();
//            querySendOrderDetailParam.setId(orderId);
//            String querySendOrderDetailParamJsonStr = JSONObject.toJSONString(querySendOrderDetailParam);
//            Map querySendOrderDetailParamsMap = JSONObject.parseObject(querySendOrderDetailParamJsonStr, Map.class);
//            Map orderParamsMap = JSONObject.parseObject(msgStr, Map.class);
//
//            //幂等性问题（即重复消费问题）解决方案：查询消息ID是否存在，存在则中断执行，不存在才继续执行。因为重试是间隔性执行同一个消息，不存在并发问题。
//            //即使收到重复消息导致并发，数据库主键是唯一的，所以只会有一条数据执行成功。
//            ResultInfo<Object> resultInfoOfQuery = apacheHttpClientUtil.doPost("http://localhost:8090/Bsystem/querySendOrder", querySendOrderDetailParamsMap);
//            if (resultInfoOfQuery != null && resultInfoOfQuery.getResultCode().equals("200") && resultInfoOfQuery.getBody() != null) {
//                return;
//            }
//
//            //做失败日志记录或者通过渠道调用业务服务

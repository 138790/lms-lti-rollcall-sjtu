...
public class ScanSignSercieImpl implements ScanSignServcie {
    ...
    public void scanUpdateState(String signScanToken) throws BusinessException, InterruptedException {
        ...
        if(StringUtils.isBlank(userStateInfo.getState())){
            throw BusinessException.notFoundData("点名历史"+signHistoryId);
        }

        //4.如果用户没有正常签到，则做更新状态操作，否则跳过
        if (!userStateInfo.getState().equals(SignInStateEnum.NORMAL.getCode())){
            ...
            if(!StringUtils.isEmpty(userStateInfo.getUserName())){
                simpMessageSendingOperations.convertAndSend("/topic/users/"+signHistoryId,userStateInfo.getUserName());
        }
